...
const DataJenisPegawai = () => {
    ...
    const [formData, setFormData] = useState({
        tipe: 'PEGAWAI', nama: '', nip: '', jabatan: '', no_hp: '', email: '', status: 'Aktif'
    });

    const fetchDataPegawai = async () => {
        setLoading(true);
        try {
            const response = await fetch('http://localhost:8000/api/data-guru-dan-karyawan?tipe=PEGAWAI');
    ...
    const handleDelete = async (id) => {
        Swal.fire({
            ...
            text: "Data pegawai terpilih akan dihapus permanen!",
            icon: 'warning',
    ...
    const openAddModal = () => {
        setIsEdit(false);
        setFormData({ tipe: 'PEGAWAI', nama: '', nip: '', jabatan: '', no_hp: '', email: '', status: 'Aktif' });
        setShowModal(true);
    };

    const openEditModal = (pegawai) => {
        setIsEdit(true);
        setSelectedId(pegawai.id);
        setFormData({
            tipe: pegawai.tipe, nama: pegawai.nama, nip: pegawai.nip, jabatan: pegawai.jabatan, no_hp: pegawai.no_hp, email: pegawai.email, status: pegawai.status
        });
        setShowModal(true);
    };
    ...
    const filteredPegawai = listPegawai.filter(item =>
            ...
            <div className="flex-1 p-6 md:p-8 mt-12">
                <div className="bg-violet-600 p-3 rounded-t-lg flex flex-col md:flex-row justify-between items-center shadow-md gap-3">
                    <h2 className="text-white font-bold flex items-center gap-2 text-lg">
                        <i className="ri-group-fill"></i> Data Pegawai
                    </h2>
                    <div className="flex flex-wrap gap-2">
                        <button className="bg-white text-black text-xs px-5 py-3 rounded flex items-center gap-1 hover:bg-gray-100 transition font-medium" onClick={openAddModal}>
                            <i className="ri-add-line"></i> Tambah Pegawai
                        </button>
                    </div>
                </div>
                ...
                <div className="bg-white p-4 shadow rounded-b-lg border border-gray-200">
                    <div className="flex w-full mb-6 border border-violet-600 rounded overflow-hidden">
                        <button onClick={() => navigate("/manajemen-kepegawaian/data-guru")} className="flex-1 py-3 text-sm font-bold bg-white text-violet-600 hover:bg-gray-50">
                            <i className="ri-user-settings-fill mr-2"></i> Guru
                        </button>
                        <button onClick={() => setActiveTab('pegawai')} className={`flex-1 py-2 text-sm font-bold flex justify-center items-center gap-2 transition ${activeTab === 'pegawai' ? 'bg-violet-600 text-white' : 'bg-white text-violet-600'}`}>
                            <i className="ri-user-settings-fill"></i> Pegawai
                        </button>
                    </div>
                        ...
                        <table className="w-full text-sm border border-gray-200 rounded-lg overflow-hidden">
                            ...
                            <tbody>
                                {loading ? (
                                    ...
                                    <tr><td colSpan="8" className="p-4 text-center text-gray-500">Tidak ada data pegawai ditemukan.</td></tr>
                                ) : (
                                    filteredPegawai.map((pegawai, index) => (
                                        <tr key={pegawai.id} className="border-b border-gray-100 hover:bg-gray-50">
                                            <td className="p-3 text-center">{index + 1}</td>
                                            <td className="p-3 font-semibold text-gray-800">{pegawai.nama}</td>
                                            <td className="p-3">{pegawai.nip}</td>
                                            <td className="p-3">{pegawai.jabatan || 'Staf / Karyawan'}</td>
                                            <td className="p-3">{pegawai.no_hp}</td>
                                            <td className="p-3">{pegawai.email}</td>
                                            <td className="p-3 text-center">
                                                <span className={`px-2 py-0.5 text-xs rounded-full ${pegawai.status === 'Aktif' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                                    {pegawai.status}
                                                </span>
                                            </td>
                ...
                <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50 animate-fadeIn">
                    <div className="bg-white w-full max-w-lg rounded-lg shadow-xl overflow-hidden">
                        <div className="flex justify-between items-center bg-gray-50 border-b p-4">
                            <h3 className="text-base flex items-center gap-1"><i className="ri-edit-circle-line"></i> {isEdit ? 'Edit Data Pegawai' : 'Tambah Data Pegawai'}</h3>
                            <button onClick={() => setShowModal(false)} className="hover:bg-violet-700 p-1 rounded-full"><i className="ri-close-line text-xl"></i></button>
                        </div>
                        <form onSubmit={handleSave} className="p-5 space-y-4">
                            <div>
                                <label className="block text-xs font-semibold text-gray-600 mb-1">Nama Lengkap</label>
                                <input type="text" required value={formData.nama} onChange={(e) => setFormData({ ...formData, nama: e.target.value })} className="w-full border rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-violet-200 focus:border-violet-600 outline-none transition" />
                            </div>
                            <div>
                            ...
                            <div>
                                <label className="block text-xs font-semibold text-gray-600 mb-1">Jabatan / Bagian</label>
                                <input type="text" value={formData.jabatan} onChange={(e) => setFormData({ ...formData, jabatan: e.target.value })} placeholder="Contoh: Tata Usaha, Keamanan" className="w-full border rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-violet-200 focus:border-violet-600 outline-none transition" />
                            </div>
                            <div>
...
export default DataJenisPegawai;